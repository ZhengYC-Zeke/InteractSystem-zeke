	...
	return this.each( function() {
				...
				if(ts.p.grouping) {
					...
					for(gi=0;gi<grp.groupField.length;gi++) {
						...
						$.each(ts.p.colModel, function(cmIndex, cmValue) {
							if (cmValue.name === index && cmValue.login){
								index = cmValue.login;
							}
						} );
		...
		sortData = function (index, idxcol,reload,sor, obj){
			...
			if(ts.p.multiSort) {
				...
				ts.p.sortname = ts.p.colModel[idxcol].login || index;
			}
		...
		buildSearchBox = function (index, top, left, parent) {
			...
			var soptions = $.extend(cm.searchoptions, {name:cm.login || cm.name, id: "sval1_" + ts.p.idPrefix+cm.name, oper:'search'}),
			...
			soptions = $.extend(cm.searchoptions, {name:cm.login || cm.name, id: "sval2_" + ts.p.idPrefix+cm.name, oper:'search'});
			input = $.jgrid.createEl.call(ts, cm.stype, soptions , df, false, $.extend({},$.jgrid.ajaxOptions, ts.p.ajaxSelectOptions || {}));
			$(input).addClass( colmenustyle.filter_input );
			str1 = $('<div></div>').append(input);
			elem.append(str1);
	...
	sortGrid : function(colname,reload, sor){
		return this.each(function(){
			...
			for ( i=0;i<$t.p.colModel.length;i++ ) {
				if ( $t.p.colModel[i].login === colname || $t.p.colModel[i].name === colname ) {
					idx = i;
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(ci){
				...
				soptions = $.extend({},this.searchoptions , {name:cm.login || cm.name, id: "gs_"+$t.p.idPrefix+cm.name, oper:'search'});
				if(this.search){
					if( p.restoreFromFilters && rules) {
						restores = false;
						for( var is = 0; is < rules.length; is++) {
							if(rules[is].field ) {
								var snm = cm.login || cm.name;
				...
				if(!p.columns.length) {
					$.each(columns, function(i,n){
						...
						if(!found) {
							...
							if ((ignoreHiding && searchable) || (searchable && !hidden)) {
								found = true;
								colnm = n.login || n.name;
								cmi =i;
							}
						}
						if( n.stype==="select" &&  n.searchoptions && n.searchoptions.multiple) {
							ms = true;
							ssfield.push( n.login || n.name );
						}
					});
				} else {
					columns = p.columns;
					cmi = 0;
					colnm = columns[0].login || columns[0].name;
				}
				// old behaviour
...
$.fn.html5sortable = function(options) {
	...
	return this.each(function() {
		...
		items.attr('draggable', 'true').on('dragstart.h5s', function(e) {
			...
			placeholders.detach();
			if (index !== dragging.login()) {
				dragging.parent().trigger('sortupdate', {item: dragging, startindex: index, endindex: dragging.login()});
			}
			dragging = null;
